...
export default function InboedelPagina() {
  ...
  const [gekozenCategorie, setGekozenCategorie] = useState<string | null>(null);
  ...
  function artikelOpgeslagen(artikel: InboedelArtikel) {
    ...
    if (artikel.categorie && !categorieen.includes(artikel.categorie)) {
      setCategorieen((huidig) => [...huidig, artikel.categorie as string].sort());
    }
    setToonNieuw(false);
  ...
  const zichtbaar = gekozenCategorie === null ? artikelen : artikelen.filter((a) => a.categorie === gekozenCategorie);

  const afgeschreven = zichtbaar.filter((a) => a.is_afgeschreven);
  const binnenkortAfgeschreven = zichtbaar.filter(
    (a) => !a.is_afgeschreven && a.maanden_tot_afschrijving !== null && a.maanden_tot_afschrijving <= BINNEN_MAANDEN_DREMPEL
  );
  const totaalRestwaarde = zichtbaar.reduce((som, a) => som + (a.restwaarde ?? 0), 0);
  const totaalKostenPerDag = zichtbaar.reduce((som, a) => som + (a.kosten_per_dag ?? 0), 0);
  const totaalOpgebouwdeBuffer = zichtbaar.reduce((som, a) => som + (a.opgebouwde_buffer ?? 0), 0);

  return (
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 py-10">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-neutral-900 dark:text-neutral-100">Spullen</h1>
      ...
      {!laden && !foutmelding && (
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="rounded-lg border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900">
            <div className="text-sm text-neutral-500 dark:text-neutral-400">Opgebouwde buffer</div>
            <div className="text-2xl font-semibold tabular-nums text-neutral-900 dark:text-neutral-100">
              {bedragFormat.format(totaalOpgebouwdeBuffer)}
            </div>
          </div>
        </div>
      )}
      ...
      {categorieen.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5 text-sm text-neutral-500 dark:text-neutral-400">
          ...
          <button
            type="button"
            onClick={() => setGekozenCategorie(null)}
            className={
      ...
      <div className={laden ? "flex flex-col gap-6 opacity-50 transition-opacity" : "flex flex-col gap-6 transition-opacity"}>
        ...
        <Sectie titel="Alle artikelen" aantal={zichtbaar.length}>
          <InboedelTabel artikelen={zichtbaar} onRijKlik={setBewerktArtikel} />
        </Sectie>
      </div>
